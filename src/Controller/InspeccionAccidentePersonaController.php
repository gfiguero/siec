...
use App\Form\FiltroAccidentePersonaType;
use App\Form\ExportarAccidentePersonaType;
...
/**
 * @Route("/inspeccion/accidentePersona")
 */
class InspeccionAccidentePersonaController extends AbstractController
{
    ...
    /**
     * @Route("/filtrar", name="inspeccion_accidentePersona_filtrar", methods={"GET", "POST"})
     */
    public function filtrar(Request $request): Response
    {
        $filtros = $this->get('session')->has('filtroAccidentePersona') ? $this->get('session')->get('filtroAccidentePersona') : null;

        $this->manageObjects($filtros);

        $form = $this->createForm(FiltroAccidentePersonaType::class, $filtros);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $this->get('session')->set('filtroAccidentePersona', $form->getData());
            return $this->redirectToRoute('inspeccion_accidentePersona_buscar');
        }

        return $this->render('inspeccion/accidentePersona/filtrar.html.twig', [
            'form' => $form->createView(),
        ]);
    }

    /**
     * @Route("/buscar", name="inspeccion_accidentePersona_buscar", methods={"GET"})
     */
    public function buscar(Request $request, PaginatorInterface $paginator): Response
    {
        $busqueda = array();
        if($this->get('session')->has('filtroAccidentePersona')) {
            $busqueda = $this->get('session')->get('filtroAccidentePersona');
        }

        $query = $this->getDoctrine()->getRepository(Accidente::class)->filtroAccidentePersona($busqueda);
        ...
        return $this->render('inspeccion/accidentePersona/buscar.html.twig', [
    ...
    /**
     * @Route("/exportar", name="inspeccion_accidentePersona_exportar", methods={"GET", "POST"})
     */
    public function exportar(Request $request, TranslatorInterface $translator): Response
    {
        ...
        $form = $this->createForm(ExportarAccidentePersonaType::class);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {

            $accidentes = $this->getDoctrine()->getRepository(Accidente::class)->filtroAccidentePersona($busqueda)->getResult();
            ...
            $sheet->setTitle("Accidentes por persona");
        ...
        return $this->render('inspeccion/accidentePersona/exportar.html.twig', [
            'busqueda' => $busqueda,
            'form' => $form->createView(),
        ]);
    }
