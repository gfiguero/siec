 ...
 * Cuadrante
 *
 * @ORM\Table(name="Cuadrante", uniqueConstraints={@ORM\UniqueConstraint(name="UQ_Cuadrante", columns={"Codigo"})}, indexes={@ORM\Index(name="IDX_Cuadrante_Comuna", columns={"Comuna"})})
 * @ORM\Entity(repositoryClass="App\Repository\CuadranteRepository")
 * @UniqueEntity("codigo")
 */
class Cuadrante
{
     ...
     * @var \Comuna
     *
     * @ORM\ManyToOne(targetEntity="Comuna", inversedBy="cuadrantes")
     * @ORM\JoinColumns({
     ...
     * @var \Accidentes
     *
     * @ORM\OneToMany(targetEntity="App\Entity\Accidente", mappedBy="cuadrante")
     */
    private $accidentes;
    ...
    public function removeAccidente(Accidente $accidente): self
    {
        if ($this->accidentes->contains($accidente)) {
            ...
            if ($accidente->getCuadrante() === $this) {
                $accidente->setCuadrante(null);
            }
        }
